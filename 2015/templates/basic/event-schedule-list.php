...
<div class="content header-band-bump">
  ...
  <div id="events-schedule" class="wrap content-page events-schedule <?=(count($pageNav)>1)?'has-nav':'';?>">
    ...
    <section class="the-meat">
      ...
      <? if ($day == "friday") {
        $dTitle = "Friday, August 7th";
      } elseif ($day == "saturday") {
        $dTitle = "Saturday, August 8th";
      } else {
        $dTitle = "Sunday, August 9th";
      }
      ...
      <fieldset>
        ...
        <? if ($cTime != $eStartTime): ?>
          ...
          <? $cTime = $eStartTime; ?>
        <? endif ?>
          <li style="border-color: <?=$event['color_code']?>;" data-event-name="<?=$event['event_name']?>" data-start-time="<?=date("n/j/Y g:i a",strtotime($event['start_time']))?>" data-end-time="<?=date("n/j/Y g:i a",strtotime($event['end_time']))?>" data-location="<?=$event['location_name']?>" data-track="<?=$event['track_name']?>" data-description="<?=htmlentities("<p>".$event['description']."</p>")?>">
