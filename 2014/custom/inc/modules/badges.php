<?
    ...
    private function getBadgesQuery() {
      $q = sqlquery("SELECT `ba`.`id`, `ba`.`badge_name`, `ba`.`badge_price`, 
        `ba`.`registration_url`, `ba`.`gift_card_value`, `ba`.`sold_out`, 
      ...
      $i = 0;
      while ($f = sqlfetch($q)) {
        $currentBadge = $this->get($f);
        $items[$i]['id'] = $currentBadge['id'];
        $items[$i]['badge_name'] = $currentBadge['badge_name'];
        $items[$i]['badge_price'] = $currentBadge['badge_price'];
        $items[$i]['registration_url'] = $currentBadge['registration_url'];
        $items[$i]['gift_card_value'] = $currentBadge['gift_card_value'];
        $items[$i]['sold_out'] = $currentBadge['sold_out'];
        $items[$i]['perks'] = explode('|/|', $currentBadge['perks']);
        $i++;
      }
      return $items;
    }
