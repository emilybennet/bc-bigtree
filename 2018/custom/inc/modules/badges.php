<?
    ...
    private function getBadgesQuery() {
      $q = sqlquery("SELECT `ba`.`id`, `ba`.`badge_name`, `ba`.`badge_price`,
        `ba`.`registration_url`, `ba`.`gift_card_value`, `ba`.`anypony_voucher_qty`, `ba`.`photo_text`, `ba`.`tshirt_included`, `ba`.`sale_mode`, `ba`.`sold_out`,
      ...
      while ($f = sqlfetch($q)) {
        ...
        $items[$i]['anypony_voucher_qty'] = $f['anypony_voucher_qty'];
				$items[$i]['photo_text'] = $f['photo_text'];
