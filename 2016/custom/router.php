<?
  ...
  // Custom handling feed calls.
  if ($bigtree["path"][0] == "feeds") {
    list($inc,$commands) = BigTree::route(SERVER_ROOT."templates/feeds/",array_slice($bigtree["path"],1));
    if (!file_exists($inc)) {
      header($_SERVER["SERVER_PROTOCOL"]." 404 Not Found");
      die("File not found.");
    }
    $bigtree["feed_inc"] = $inc;
    $bigtree["commands"] = $commands;

    // Get the pieces of the location so we can get header and footers. Take away the first 2 routes since they're templates/feeds.
    $pieces = array_slice(explode("/",str_replace(SERVER_ROOT,"",$inc)),2);
    // Include all headers in the module directory in the order they occur.
    $inc_path = "";
    $bigtree["feed_headers"] = $bigtree["feed_footers"] = array();
    foreach ($pieces as $piece) {
      if (substr($piece,-4,4) != ".php") {
        $inc_path .= $piece."/";
        $header = SERVER_ROOT."templates/feeds/".$inc_path."_header.php";
        $footer = SERVER_ROOT."templates/feeds/".$inc_path."_footer.php";
        if (file_exists($header)) {
          $bigtree["feed_headers"][] = $header;
        }
        if (file_exists($footer)) {
          $bigtree["feed_footers"][] = $footer;
        }
      }
    }
    // Draw the headers.
    foreach ($bigtree["feed_headers"] as $header) {
      include $header;
    }
    // Draw the main page.
    include $bigtree["feed_inc"];
    // Draw the footers.
    $bigtree["feed_footers"] = array_reverse($bigtree["feed_footers"]);
    foreach ($bigtree["feed_footers"] as $footer) {
      include $footer;
    }
    die();
  }
