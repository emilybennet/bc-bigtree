  ...
  foreach ($events as $e) {
    if ($holdingArray["event_name"] == $e['event_name']) {
      ...
      if (!empty($holdingArray)) {
        echo '<event>';
        echo '<title>'.ampRemoval($holdingArray['event_name']).'</title>'.PHP_EOL;
        ...
        foreach ($holdingArray['schedule'] as $s) {
          ...
          echo '<time>';
          echo '<start>'.date('g:sa', strtotime($s['start_time'])).'</start>&#x2013;';
          echo '<end>'.date('g:sa', strtotime($s['end_time'])).'</end> ';
          echo '</time>';
          echo '<location>'.ampRemoval($s['location_name']).'</location>';
          if ($lastItem == $s) {
            echo '</schedule>';
          }
          echo PHP_EOL;
        ...
        echo '<desc>'.ampRemoval($holdingArray['description']).'</desc>';
        echo '</event>'.PHP_EOL;
      }   
